...
<style>
    ...
    /* WADAH STRUK VISUAL (KERTAS PUTIH) */
    .struk-container {
...
@st.cache_resource
def init_db():
    conn = sqlite3.connect("warung_hade_final_v_wib.db", check_same_thread=False)
    c = conn.cursor()
...
if menu == "🛒 Kasir":
        ...
        if st.button("➕ Tambah Ke Keranjang", type="primary"):
            if nama and harga > 0:
                ...
                st.session_state.keranjang.append({
                    "Nama": nama, "Harga": int(harga), "Qty": int(qty), "Subtotal": int(subtotal)
                })
                st.rerun()
    ...
    if st.session_state.keranjang:
        ...
        if st.button("✅ PROSES PEMBAYARAN", type="primary"):
            if bayar >= total:
                ...
                waktu_sekarang = datetime.now(tz_indo).strftime("%d/%m/%Y %H:%M")
                
                c = conn.cursor()
                c.execute("INSERT INTO transaksi (waktu,total_belanja,bayar,kembali) VALUES (?,?,?,?)", 
                          (waktu_sekarang, int(total), int(bayar), int(kembali)))
                trx_id = c.lastrowid
                ...
                st.session_state.last_trx = {
                    "waktu": waktu_sekarang, "items": st.session_state.keranjang.copy(),
    ...
    if st.session_state.last_trx:
        ...
        for item in trx['items']:
            rows_html += f"<tr><td style='padding:5px 0; border-bottom:1px dashed #ddd;'>{item['Nama']}<br><small style='color:#555;'>{item['Qty']} x {format_rupiah(item['Harga'])}</small></td><td style='text-align:right; vertical-align:bottom; border-bottom:1px dashed #ddd;'>{format_rupiah(item['Subtotal'])}</td></tr>"
...
# =====================================================
    ...
    if not df.empty:
        ...
        c1, c2 = st.columns(2)
        c1.metric("Transaksi", f"{len(df)} Bon")
        c2.metric("Omzet", format_rupiah(total_omzet))
        st.dataframe(df, use_container_width=True, hide_index=True)
        buffer = io.BytesIO()
